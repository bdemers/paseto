...
const methods = {
  ...
  async sign (alg, payload, key) {
    if (key.key) {
      return wrapBuffer(await subtle.sign(alg, key.key, payload))
    } else {
      const sodiumKey = sodium.crypto_sign_seed_keypair(key)
      return wrapBuffer(sodium.crypto_sign_detached(payload, sodiumKey.privateKey))
    }
  },
